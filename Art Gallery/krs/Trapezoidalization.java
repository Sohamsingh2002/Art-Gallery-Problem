...
public class Trapezoidalization {
    ...
    public DCEL dcel;
    public ArrayList<LineSegment> trapezoidalLines;

    public void generateTrapezoid() {
        for (Edge edge : dcel.edges) {
            Vertex v1 = edge.origin;
            Vertex v2 = edge.dest;
            Vertex v3 = edge.nextEdge.dest;
            v2.type = getVertexType(v1, v2, v3);
        }
        ...
        for (Vertex v : dcel.vertices) {
            if (v.type == vType.start) {
                ...
                SList.add(v.incidentEdge.prevEdge);
            } else if (v.type == vType.end) {
                SList.remove(v.incidentEdge);
                SList.remove(v.incidentEdge.prevEdge);
            } else if (v.type == vType.split) {
                Edge be1 = v.incidentEdge;
                Edge be2 = v.incidentEdge.prevEdge;
                ...
                trapezoidalLines.add(new LineSegment(lIntersection, rIntersection));
                ...
                Edge be = v.incidentEdge.prevEdge;
                ...
                SList.add(v.incidentEdge);
                Point rIntersection = getYIntersection(next, v.coord.y);
                trapezoidalLines.add(new LineSegment(v.toPoint(), rIntersection));
    ...
    public vType getVertexType(Vertex v1, Vertex v2, Vertex v3) {
